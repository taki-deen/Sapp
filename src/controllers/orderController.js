...
const createOrder = async (req, res) => {
  ...
  try {
    const { serviceType, description, location, scheduledTime, notes } =
      req.body;
    ...
    res
      .status(500)
      .json({ message: "Error creating service request", error: err.message });
  }
};
...
const getAllOrders = async (req, res) => {
  try {
    ...
    if (req.user.role === "customer") {
      ...
      filter.$or = [{ workerId: req.user._id }, { status: "pending" }];
    }
    ...
    if (req.query.serviceTypeName) {
      filter.serviceTypeName = req.query.serviceTypeName;
    }
    ...
    res
      .status(500)
      .json({ message: "Error fetching service requests", error: err.message });
  }
};
...
const getOrderById = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error fetching service request", error: err.message });
  }
};
...
const assignWorker = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error assigning worker", error: err.message });
  }
};
...
const rateWorker = async (req, res) => {
  ...
  try {
    const order = await orderService.rateWorker(
      req.params.id,
      req.user._id,
      req.body.rating
    );
    ...
    res
      .status(500)
      .json({ message: "Error rating worker", error: err.message });
  }
};
...
const deleteOrder = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error deleting service request", error: err.message });
  }
};
